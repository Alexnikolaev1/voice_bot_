...
const { resendCachedFile } = require('../max/uploads');
const { getLastAudioId } = require('../storage/audioCache');
...
async function handleCallback(update) {
  ...
  if (payload.startsWith('dl:')) {
    const audioId = payload.slice(3);
    await answerCallback(callbackId, { notification: 'Отправляю файл…' });
    if (audioId === 'last') {
      const lastId = await getLastAudioId(storageKey);
      if (!lastId) {
        await sendMessage(target, { text: 'Нет последней озвучки. Отправьте текст.' });
        return;
      }
      await resendCachedFile(target, lastId);
      return;
    }
    await resendCachedFile(target, audioId);
    return;
  }
  ...
  if (payload === 'menu:voices') {
    ...
    await sendMessage(target, { text: '**Выбери голос**', format: 'markdown', attachments: [voiceMenuKeyboard()] });
    return;
  }
